...
경합, Redis fallback·재구성 및 Kafka 장애 복구를 관측하는 정책을 확정했으며, 실행 도구와 스크립트는 후속
이슈에서 추가한다.
...
일반 혼합 요청, 동일 사용자 경합, Redis 장애·데이터 유실, Kafka 중단·복구를 각각 재현한다. k6의 지연·오류와
컨테이너 자원, Outbox `PENDING`, Kafka `popular-menu` consumer group lag, MySQL 원본과 인기 메뉴 API 결과를
대조한다. 각 실행은 세 번 반복하고 다음 위치에 원본과 판정 보고서를 함께 남긴다.
...
지표·판정, 병목 후보와 JSON SHA-256을 기록한다. 구체적인 시나리오, 성공·실패 조건과 안전한 데이터 정리 절차는
[PRD의 M9 정책](docs/PRD.md#p2-m9-부하-테스트와-운영-관측-정책)과
