...
P1 M7 consumer 중복 처리, P2 M8 Redis fallback·재구성의 관계, 타입, 제약과 인덱스를 정의한다. MySQL을 메뉴,
포인트와 주문의 단일 원본으로 사용하며 Redis는 재구성 가능한 인기 메뉴 조회 모델로만 사용한다.
...
| P2 M8 Redis fallback·재구성 | `orders`, `menus` | MySQL 집계 원본과 Redis 파생 모델 복구 |
...
### 5.7 P2 M8 MySQL fallback과 Redis 재구성

M8의 MySQL 원본은 조회일 포함 최근 7개 UTC 날짜의 결제 완료 `orders`다. fallback과 재구성은
`ordered_at`의 UTC 날짜를 기준으로 `D-6 00:00:00Z` 이상 `D+1 00:00:00Z` 미만을 집계하며,
`idx_orders_ordered_at_menu_id`를 사용한다. 메뉴 이름과 가격은 기존 인기 메뉴 조회와 같이 현재 `menus`에서
읽는다. 새 영속 테이블이나 인기 집계 테이블은 만들지 않는다.

여러 인스턴스는 MySQL 명명 잠금 `popular-menu-rebuild`를 비대기로 획득해 한 인스턴스만 재구성한다. 잠금을
획득하지 못한 요청은 MySQL fallback 결과를 반환하고 Redis를 쓰지 않는다. 명명 잠금은 DB 연결에 귀속되므로
재구성 종료 시 같은 연결에서 해제하며, 연결 단절은 MySQL의 잠금 해제로 처리한다.

잠금 소유자는 Redis 재구성 표식을 만들고, 점수·상태·처리 표식을 모두 다시 쓴 뒤에만 표식을 지운다. 표식이
있는 동안 조회 API는 Redis를 읽지 않고 MySQL fallback으로 응답한다. consumer는 Redis 원자 스크립트에서 표식을
확인해 Redis 갱신과 Kafka offset commit을 보류한다. 따라서 재구성의 MySQL 원본 집계에 포함된 주문은 처리
표식으로 중복 증가를 막고, 집계 이후 도착한 이벤트는 재구성 완료 뒤 한 번만 반영한다.
...
| 상태 자료구조 | String, 값은 `READY` 또는 `EMPTY` |
| 상태 key | `popular:menu:state:<yyyy-MM-dd>` |
| 처리 표식 자료구조 | String, 값은 처리 여부만 나타내는 `1` |
| 처리 표식 key | `popular:menu:processed:<yyyy-MM-dd>:<orderId>` |
| 재구성 표식 key | `popular:menu:rebuilding`, 값은 잠금 소유자 token, 유한 TTL 적용 |
...
| 갱신 | M8 전에는 Redis 원자 스크립트가 `SET ... NX PXAT` 성공 시에만 `ZINCRBY 1 <menuId>` 실행 |
| M8 갱신 | 재구성 표식이 없을 때만 기존 원자 처리와 `READY` 상태 key·동일 만료 시각을 함께 갱신 |
| 만료 | 점수 key와 처리 표식 모두 해당 날짜 `D`의 `D+8 00:00:00Z`에 만료 |
| 상태 만료 | 상태 key도 해당 날짜 `D`의 `D+8 00:00:00Z`에 만료 |
...
M8에서 Redis 조회 모델은 일자별 상태 key가 있어야 유효하다. `EMPTY` 상태에는 점수 key가 없어야 하고,
`READY` 상태에는 점수 key가 있어야 한다. 상태 key가 없거나 이 조합이 맞지 않으면 해당 날짜 주문이 없다고
추정하지 않고 MySQL fallback과 재구성 대상으로 처리한다. 재구성은 점수와 상태뿐 아니라 대상 기간의 모든
`orderId` 처리 표식을 같은 만료 시각으로 복원한다. 따라서 Redis 유실 뒤 지연된 Kafka 재전달도 이미 MySQL
집계에 포함된 주문의 점수를 다시 증가시키지 않는다.
