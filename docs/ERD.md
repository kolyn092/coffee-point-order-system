...
사용자별 현재 포인트 잔액의 원본이다. 첫 충전 성공 시 행을 만든다.
...
행이 없는 상태에서는 비관적 잠금을 획득할 수 없으므로 첫 충전은 다음 순서를 사용한다.

1. `user_id` PK를 이용해 잔액 0인 행 생성을 시도한다.
2. 동시 생성으로 duplicate key가 발생하면 기존 행을 사용한다.
3. 확정된 행을 `SELECT ... FOR UPDATE`로 조회한다.
4. checked arithmetic으로 충전 후 잔액을 계산하고 갱신한다.
5. 트랜잭션을 commit한다.

PK 제약과 DB 잠금이 모든 인스턴스에 공통으로 적용되므로 동시 첫 충전과 이후 충전이 유실되지 않는다.
