...
| 인증·인가 | P0에서는 지원하지 않음 |
| 성공 응답 | 공통 wrapper 없이 각 API의 DTO 또는 배열을 직접 반환 |
...
클라이언트는 `message`가 아니라 `code`를 분기 기준으로 사용한다. 내부 예외명, SQL, stack trace와 비밀값은
응답에 포함하지 않는다.
...
| `[].menuId` | Long | 메뉴 ID |
| `[].name` | String | 메뉴 이름 |
| `[].price` | Long | 원 단위 가격이며 같은 수의 포인트가 필요함 |

메뉴가 없으면 `200 OK`와 빈 배열 `[]`을 반환한다.
...
| Name | Type | Description |
| --- | --- | --- |
| `userId` | String | 충전 대상 사용자 식별값 |
| `chargedAmount` | Long | 이번 요청에서 충전한 포인트 |
| `balance` | Long | 이번 충전 트랜잭션이 반영된 직후 잔액 |
...
| `paidAmount` | Long | 결제 당시 메뉴 가격과 실제 차감 포인트 |
| `remainingPointBalance` | Long | 주문 트랜잭션이 반영된 직후 잔액 |
| `orderedAt` | String | 주문 완료 시각, ISO 8601 UTC |
...
| --- | --- | --- |
| `[].menuId` | Long | 메뉴 ID |
| `[].name` | String | 현재 메뉴 이름 |
| `[].price` | Long | 현재 메뉴 가격 |
| `[].orderCount` | Long | 최근 7개 UTC 날짜의 결제 완료 주문 수 |

집계 대상 주문이 없으면 `200 OK`와 빈 배열 `[]`을 반환한다.
